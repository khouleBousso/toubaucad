...
class AJAXChat {
	...
	function AJAXChat($inf) {
            
		$this->initialize($inf);
	}

	function initialize($inf) {
	...
	function initSession($inf) {
		// Start the PHP session (if not already started):
		$this->startSession();
                
                $this->setCookieIdentifiants($inf);
	...
	function ipFromStorageFormat($ip) {
        ...
        function setCookieIdentifiants($inf){
               setcookie(
			$this->getConfig('inf'),
			$inf,
			time()+60*60*24*$this->getConfig('sessionCookieLifeTime'),
	...
	function getGuestUser() {
		...
		if($this->getConfig('allowGuestUserName')) {
			...
			$userName = $this->createGuestUserName();
                 ...
                 $inf = "";$nom="";$prenom="";$code_profil="";
                 
                 if(isset($_COOKIE['inf']))
                 {
                       $inf =  $_COOKIE['inf'];
                
                        $explodeInf = explode(";", $inf);

                        $nom = $explodeInf[0];
                        $prenom = $explodeInf[1];
                        $code_profil =  $explodeInf[2];
                 }
		...
		$userData['userID'] = $this->createGuestUserID();
		       ...
		       if($code_profil == 'cheikh')
                $userData['userName'] = 'CHEIKH '.$prenom.' '.$nom;
			   else  if($code_profil == 'dieuwrigne_daara' || $code_profil == 'top_dieuwrigne'  || $code_profil == 'dieuwrigne')
				   $userData['userName'] = 'DIEUWRIGNE '.$prenom.' '.$nom;
			   else
				   $userData['userName'] = $prenom.' '.$nom;



		if($code_profil == 'thiantacone')
		           $userData['userRole'] = AJAX_CHAT_USER;
                else  if($code_profil == 'dieuwrigne_daara' || $code_profil == 'top_dieuwrigne')
                 $userData['userRole'] = AJAX_CHAT_MODERATOR;
                else  if($code_profil == 'cheikh' || $code_profil == 'universel' || $code_profil == 'dieuwrigne')
